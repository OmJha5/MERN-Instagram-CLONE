...
export default function Signup() {
  ...
  let submitHandler = async(e) => {
      ...
      try{
          setLoading(true);
          let res = await axios.post(`${USER_API_ENDPOINT}/register` , input , {
              headers : {
    ...
    <div className='flex items-center justify-center w-screen h-screen'>
        <form onSubmit={submitHandler} className="shadow-lg flex flex-col gap-5 p-8">
            ...
            <div>
                ...
                <Input placeholder="Enter your username" id="username" name="username" className="my-2" value={input.username} onChange={changeHandler} />
            </div>
            <div>
            ...
            <p className="my-2 text-sm">Already have an account ? <span className='text-blue-600'><Link to="/login">Login</Link></span></p>
        </form>
    </div>
  )
}
